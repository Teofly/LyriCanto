...
struct AppColorScheme: Codable, Equatable {
    ...
    static let defaultLight = AppColorScheme(
        name: "Chiaro (Default)",
        background: CodableColor(nsColor: NSColor.windowBackgroundColor),
        secondaryBackground: CodableColor(nsColor: NSColor.controlBackgroundColor),
        text: CodableColor(nsColor: NSColor.labelColor),
        secondaryText: CodableColor(nsColor: NSColor.secondaryLabelColor),
        accent: CodableColor(nsColor: NSColor.systemBlue),
        buttonBackground: CodableColor(nsColor: NSColor.controlColor),
        buttonText: CodableColor(nsColor: NSColor.controlTextColor)
    )
    ...
    private init() {
        if let saved = UserDefaults.standard.data(forKey: "colorScheme"),
           let decoded = try? JSONDecoder().decode(AppColorScheme.self, from: saved) {
            ...
            self.currentScheme = .defaultLight
        }
    }
