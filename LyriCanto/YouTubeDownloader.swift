...
class YouTubeDownloader: ObservableObject {
    ...
    func searchYouTube(query: String) async throws -> [YouTubeVideo] {
        ...
        do {
            ...
            if !stderrOutput.isEmpty {
                print("yt-dlp stderr: \(stderrOutput)")
            }
            
            guard process.terminationStatus == 0 else {
                throw DownloadError.downloadFailed("Ricerca fallita con status \(process.terminationStatus): \(stderrOutput)")
            }
            
            let data = pipe.fileHandleForReading.readDataToEndOfFile()
            let output = String(data: data, encoding: .utf8) ?? ""
            
            print("yt-dlp output length: \(output.count) bytes")
    ...
    private func parseYtDlpJsonOutput(_ output: String) -> [YouTubeVideo] {
        ...
        print("Parsing \(lines.count) lines of JSON")
        
        for (index, line) in lines.enumerated() {
            guard let jsonData = line.data(using: .utf8) else {
                print("Skipping line \(index): could not convert to data")
                continue
            }
            ...
            do {
                ...
                // Costruisci thumbnail URL manualmente usando l'ID del video
                // YouTube usa una struttura standard per i thumbnail
                let thumbnailURL = "https://i.ytimg.com/vi/\(videoInfo.id)/mqdefault.jpg"
                ...
                videos.append(video)
                print("Parsed video: \(videoInfo.title) (ID: \(videoInfo.id))")
            } catch {
                print("Errore parsing JSON video at line \(index): \(error)")
                print("Line content: \(line.prefix(200))")
                continue
            }
        }
